...
and connect it to the REST API you created in [Part 3:](../tandt-rest-api/README.md).

All steps are carried out on the Cloud9 instance you created in [Part 1](../tandt-fabric/README.md).
...
by AWS CloudFormation in [Part 1](../tandt-fabric/README.md). You can find the DNS endpoint for the ELB in
...
```
cd ~
git clone https://github.com/aws-samples/trackandtrace.git
```
...
by CloudFormation in [Part 1](../tandt-fabric/README.md). You can find the DNS endpoint for the ELB in
...
See the function `dummySpend()` in `tandt-rest-api/app.js` for details on how spend records are randomly created.
...
into the network, such as [this one](../tandt-rest-api/ngo-load-workshop.sh)

You could also query the chaincode directly and see the same information. You query the chaincode from 
the Fabric client node. An example of how to query the chaincode can be found in [this script](../tandt-chaincode/test-chaincode-aws.sh)
