...
export async function fetchSettlementContracts(dispatch, token, setIsFetching, setError) {
  setError(false);
  setIsFetching(true);
    ...
    try {
      const options = { 
          ...
          "%templates" : [
            {
              "moduleName" : "Main",
              "entityName" : "SettlementTrades"
            }
          ],
          "tradeStage": "Settlement"
        };
      const response = await fetch("/contracts/search", options);
...
function tradeTemplateFilterSDK(contract, moduleName, entityName) {
  return contract.templateId.entityName !== 'SettlementTrades';
}

function settlementTemplateFilterSDK(contract, moduleName, entityName) {
  return contract.templateId.entityName === 'SettlementTrades' || contract.templateId.entityName === 'ColletralTrade';
}

export function getSettlementContracts(state, moduleName, entityName) {
  const templateFilter = config.isLocalDev ? settlementTemplateFilterSDK : templateFilterDABL
  return state.contracts.filter(c => templateFilter(c, moduleName, entityName));
}

export function getTradeContracts(state, moduleName, entityName) {
  const templateFilter = config.isLocalDev ? tradeTemplateFilterSDK : templateFilterDABL
  return state.contracts.filter(c => templateFilter(c, moduleName, entityName));
}
