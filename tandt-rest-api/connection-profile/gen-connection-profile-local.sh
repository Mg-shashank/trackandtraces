...
# This script uses the template tandt-connection-profile.json to generate a connection profile
# for the organisations in the Fabric network.

#REPODIR points to this repo.
REPODIR=~/Documents/apps/trackandtrace
...
cp tandt-connection-profile.yaml $REPODIR/tmp/connection-profile
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    sed -e "s|%REPODIR%|$CERTDIR|g" tandt-connection-profile.yaml > $REPODIR/tmp/connection-profile/tandt-connection-profile.yaml
else
    sed -i "s|%REPODIR%|$CERTDIR|g"  $REPODIR/tmp/connection-profile/tandt-connection-profile.yaml
fi 
